...
ARG HOST_UID=1000
ARG HOST_GID=1000
RUN addgroup --gid ${HOST_GID} flufffoxng && \
  adduser -S --shell /bin/sh --uid ${HOST_UID} flufffoxng && \
  addgroup flufffoxng wheel && \
  echo "flufffoxng ALL=(ALL) NOPASSWD:ALL" >> /etc/sudoers
