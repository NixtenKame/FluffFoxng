...
**Port already in use**: You have another instance of FluffFoxng (or something else using the same port) open. Stop it, or you can try editing `.env.devcontainer`.
### Service failures
...
**Docker Compose not working**: Docker compose is not available in the container. `docker-cli-compose` causes the FluffFox service to be unable to find the `Procfile`.
