...
class SensitiveParamsTest < ActiveSupport::TestCase
  def assert_match_group(expected, query)
    assert_equal(expected, query[SensitiveParams.to_query_filter_regex])
  end

  def assert_no_match_group(query)
    assert_nil(query[SensitiveParams.to_query_filter_regex])
  end
