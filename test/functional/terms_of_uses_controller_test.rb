...
class TermsOfUsesControllerTest < ActionDispatch::IntegrationTest
  context "The terms of use controller" do
    setup do
    ...
    context "accept action" do
      ...
      should "not set the cookie and show a notice when declined" do
        ...
        assert_match(/You must accept the TOU/, flash[:notice])
      end
    ...
    context "clear_cache action" do
      ...
      should "require admin and clear the cached content" do
        ...
        post_auth clear_cache_terms_of_use_path, @admin
        assert_redirected_to terms_of_use_path
        assert_equal "Terms of use cache cleared", flash[:notice]
      end
    end
    ...
    context "bump_version action" do
      ...
      should "increment the version, clear cache, and redirect for admins" do
        ...
        post_auth bump_version_terms_of_use_path, @admin

        assert_redirected_to terms_of_use_path
        assert_equal 11, Setting.tos_version.to_i
        assert_equal "Terms of use version bumped to 11", flash[:notice]
      end
    end
  end
end
